...
RSpec.describe "Layout shell", type: :feature do
  ...
  # let(:adam)         { create(:user, role: :admin) }            # Adam Admin - A signed in user with the Admin user role - NOTE: Not required for this spec
  # let(:sam)          { create(:user, role: :system_admin) }     # Sam SysAdmin - A signed in user with the System Admin user role - NOTE: Not required for this spec
