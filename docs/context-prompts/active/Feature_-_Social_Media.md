...
* Existing (persisted) rows are removed via a plain, always-visible `_destroy` checkbox — no JS required for that at all, it's a normal form submit (matches the "native elements, no custom widgets" accessibility decision better than the originally planned hidden-field+JS-hide approach). Only "Add Another Platform" needs JS — it clones a `<template>` row (`social_media_accounts_controller.js`, new Stimulus controller — no existing pattern fits, `author_shuttle_controller.js` moves whole options between two lists rather than carrying a free-text value per row). A freshly-added, not-yet-saved row's own "Remove" is a second JS action that just deletes the DOM node (nothing to reach the server for, since it was never persisted).
